...
def main(args):
	...
	vocab_src = utils.read_pkl('./de-en/nmt_simple.src.vocab.pkl')
	# vocab_src.update({'<SOS>' : 35819})
	vocab_tgt = utils.read_pkl('./de-en/nmt_simple.tgt.vocab.pkl')
	vocab_tgt.update({'<SOS>' : 24999})

	tr_dataset = dataloader.NMTSimpleDataset(max_len=args.max_len,
											src_filepath='./de-en/nmt_simple.src.train.txt',
											tgt_filepath='./de-en/nmt_simple.tgt.train.txt',
											vocab=(vocab_src, vocab_tgt))
	ts_dataset = dataloader.NMTSimpleDataset(max_len=args.max_len,
											src_filepath='./de-en/nmt_simple.src.test.txt',
	...
	with open('./de-en/nmt_simple_len.tgt.test.npy', 'rb') as f:
		lengths = np.load(f)
